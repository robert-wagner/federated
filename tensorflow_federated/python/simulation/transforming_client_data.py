...
class TransformingClientData(client_data.ClientData):
  ...
  def __init__(self, raw_client_data, transform, num_transformed_clients):
    ...
    Args:
      ...
      transform: A function f(x, i) mapping datapoint x to a new datapoint
        parameterized by i.
      num_transformed_clients: The number of transformed clients to produce. If
        it is an integer multiple k of the number of real clients, there will be
    ...
    if num_transformed_clients <= 0 or math.isinf(num_transformed_clients):
    ...
    for c in range(num_extra_client_ids):
      self._client_ids.append('{}_{}'.format(raw_client_ids[c], k))
